...
/**
 * Tests the AmazonSQSExtendedAsyncClient class.
 */
public class AmazonSQSExtendedAsyncClientTest {
    ...
    private SqsAsyncClient extendedSqsWithDeprecatedMethods;
    ...
    public void setupClients() {
        ...
        extendedSqsWithDeprecatedMethods = spy(new AmazonSQSExtendedAsyncClient(mockSqsBackend, extendedClientConfigurationDeprecated));
    }
    ...
    public void testWhenSendMessageWithAlwaysThroughS3AndMessageIsSmallThenItIsStillStoredInS3WithDeprecatedMethod() {
        ...
        SqsAsyncClient sqsExtended = spy(new AmazonSQSExtendedAsyncClient(mock(SqsAsyncClient.class), extendedClientConfiguration));
    ...
    public void testReceiveMessageMultipleTimesDoesNotAdditionallyAlterReceiveMessageRequestWithDeprecatedMethod() {
        ExtendedClientConfiguration extendedClientConfiguration = new ExtendedClientConfiguration()
                .withLargePayloadSupportEnabled(mockS3, S3_BUCKET_NAME);
        SqsAsyncClient sqsExtended = spy(new AmazonSQSExtendedAsyncClient(mockSqsBackend, extendedClientConfiguration));
        when(mockSqsBackend.receiveMessage(isA(ReceiveMessageRequest.class))).thenReturn(CompletableFuture.completedFuture(ReceiveMessageResponse.builder().build()));
    ...
    public void testSendLargeMessageWithDefaultConfigThenLegacyReservedAttributeNameIsUsed(){
        ...
        Assert.assertTrue(attributes.containsKey(AmazonSQSExtendedAsyncClient.LEGACY_RESERVED_ATTRIBUTE_NAME));
    ...
    public void testSendLargeMessageWithGenericReservedAttributeNameConfigThenGenericReservedAttributeNameIsUsed(){
        ...
        Assert.assertFalse(attributes.containsKey(AmazonSQSExtendedAsyncClient.LEGACY_RESERVED_ATTRIBUTE_NAME));
    }
    ...
    public void testReceiveMessage_when_MessageIsLarge_legacyReservedAttributeUsed() throws Exception {
        testReceiveMessage_when_MessageIsLarge(AmazonSQSExtendedAsyncClient.LEGACY_RESERVED_ATTRIBUTE_NAME);
    }
    ...
    public void testReceiveMessage_when_MessageIsSmall() throws Exception {
        ...
        Message message = Message.builder()
            ...
            .build();
        when(mockSqsBackend.receiveMessage(isA(ReceiveMessageRequest.class))).thenReturn(CompletableFuture.completedFuture(ReceiveMessageResponse.builder().messages(message).build()));

        ReceiveMessageRequest messageRequest = ReceiveMessageRequest.builder().build();
        ReceiveMessageResponse actualReceiveMessageResponse = extendedSqsWithDefaultConfig.receiveMessage(messageRequest).get();
        ...
        Assert.assertFalse(actualMessage.messageAttributes().keySet().containsAll(AmazonSQSExtendedAsyncClient.RESERVED_ATTRIBUTE_NAMES));
    ...
    public void testWhenLargeMessageIsSentThenAttributeWithPayloadSizeIsAdded() {
        ...
        Map<String, MessageAttributeValue> attributes = sendMessageRequestCaptor.getValue().messageAttributes();
        assertEquals("Number", attributes.get(AmazonSQSExtendedAsyncClient.LEGACY_RESERVED_ATTRIBUTE_NAME).dataType());
        assertEquals(messageLength, (int) Integer.parseInt(attributes.get(AmazonSQSExtendedAsyncClient.LEGACY_RESERVED_ATTRIBUTE_NAME).stringValue()));
    }
